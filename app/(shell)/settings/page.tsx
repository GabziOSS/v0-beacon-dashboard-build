...
import { THEMES, type ThemeId, type ThemeMode, applyTheme, getStoredTheme, getStoredMode, getEffectiveMode, initThemeListener } from "@/lib/theme"
...
const MODE_OPTIONS: Array<{ value: ThemeMode; label: string; description: string; icon: typeof Monitor }> = [
...
export default function SettingsPage() {
  ...
  const [currentTheme, setCurrentTheme] = useState<ThemeId>("civicpulse")
  const [currentMode, setCurrentMode] = useState<ThemeMode>("auto")
  const [effectiveMode, setEffectiveMode] = useState<"dark" | "soft" | "light">("dark")
  const [defaultPreset, setDefaultPreset] = useState<PresetId>("overview")
  const [persistLayout, setPersistLayout] = useState(true)
  ...
  useEffect(() => {
    ...
    setDefaultPreset(getStoredPreset())
    setPersistLayout(localStorage.getItem("beacon_persist_layout") !== "false")
    setDenseMode(localStorage.getItem("beacon_dense_mode") === "true")
    setAlertThreshold(localStorage.getItem("beacon_alert_threshold") || "high")
    setSoundAlerts(localStorage.getItem("beacon_sound_alerts") !== "false")
    setDesktopNotif(localStorage.getItem("beacon_desktop_notif") === "true")
    ...
    const unsubscribe = initThemeListener(() => {
      ...
      if (mode === "auto") {
        setEffectiveMode(getEffectiveMode("auto"))
      }
    })
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                {MODE_OPTIONS.map(({ value, label, description, icon: Icon }) => (
                  ...
                  >
                    ...
                    <span className="text-[9px] text-muted-foreground text-center leading-tight">{description}</span>
                    {currentMode === value && (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Which dashboard view to load on startup</p>
                </div>
                <select
                  value={defaultPreset}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Save block positions to localStorage</p>
                </div>
                <Toggle checked={persistLayout} onChange={v => { setPersistLayout(v); localStorage.setItem("beacon_persist_layout", String(v)) }} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-foreground">Dense mode</p>
                  <p className="text-[10px] text-muted-foreground">Reduce card padding and text size</p>
                </div>
                <Toggle checked={denseMode} onChange={v => { setDenseMode(v); localStorage.setItem("beacon_dense_mode", String(v)) }} />
              </div>
            </div>
          </section>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Minimum severity for notifications</p>
                </div>
                <select
                  value={alertThreshold}
                  onChange={e => { setAlertThreshold(e.target.value); localStorage.setItem("beacon_alert_threshold", e.target.value) }}
                  className="h-8 px-2 text-xs bg-input border border-border rounded-sm text-foreground"
                >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Play audio for incoming alerts</p>
                </div>
                <Toggle checked={soundAlerts} onChange={v => { setSoundAlerts(v); localStorage.setItem("beacon_sound_alerts", String(v)) }} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                ...
                <Toggle checked={desktopNotif} onChange={v => { setDesktopNotif(v); localStorage.setItem("beacon_desktop_notif", String(v)) }} />
              </div>
            </div>
          </section>
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-6">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-card border border-border rounded-sm p-4">
            <h3 className="text-sm font-semibold text-foreground mb-4">Appearance</h3>
            <p className="text-xs text-muted-foreground mb-3">Select a theme for the dashboard</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {(Object.entries(THEMES) as [ThemeId, typeof THEMES[ThemeId]][]).map(([id, theme]) => (
                <button
                  key={id}
                ...
                >
                  ...
                  <div className="flex gap-1 mb-2">
                    <div className="w-4 h-4 rounded-sm" style={{ background: theme.colors.bg }} />
                    <div className="w-4 h-4 rounded-sm" style={{ background: theme.colors.primary }} />
