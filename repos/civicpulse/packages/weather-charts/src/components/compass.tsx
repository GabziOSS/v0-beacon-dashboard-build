...
export function CompassChart({ bearing, label }: { bearing: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 w-full">
      <div className="relative w-full flex-1 min-h-0 aspect-square flex items-center justify-center">
        <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-sm max-w-[320px]" aria-hidden>
          <defs>
          ...
          {/* Cardinal labels */}
          {CARDINALS.map(c => {
            const p = polarXY(CX, CY, RING_R + 10, c.angle)
            return (
              <text
                key={c.label}
                x={p.x}
          ...
          <g transform={`rotate(${animBearing}, ${CX}, ${CY})`} filter="url(#compass-glow)">
