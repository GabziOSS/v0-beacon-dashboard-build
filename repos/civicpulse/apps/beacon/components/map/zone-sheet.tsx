...
import { useIncidents } from "@civicpulse/hooks"
...
function Badge({ label, className }: { label: string; className: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-sm border px-1.5 py-0.5 font-mono text-[10px] font-medium",
        className
      )}
    >
      {label}
...
export function ZoneSheet({ zone, onClose }: ZoneSheetProps) {
  ...
  return (
    <div className="fixed top-0 right-0 bottom-0 z-50 flex w-[420px] flex-col border-l border-border bg-card shadow-2xl">
      {/* Header */}
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          >
            <MapPin className={cn("h-4 w-4", RISK_TEXT[zone.risk])} />
          </div>
          <div className="min-w-0">
            ...
            <p className="font-mono text-[10px] text-muted-foreground">
              {zone.district} · {zone.population.toLocaleString()} pop.
            </p>
          </div>
        </div>
        ...
        {TABS.map((t) => (
          ...
          >
            ...
            {t === "Incidents" && zoneIncidents.length > 0 && (
              <span className="ml-1.5 rounded-sm bg-secondary px-1 py-0.5 font-mono text-[9px] text-muted-foreground">
          ...
          <div className="space-y-4 p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {
                  label: "Responders",
                  value: zoneIncidents.reduce((s, i) => s + i.responders, 0),
                  color: "text-foreground",
                },
              ].map((kpi) => (
                <div
                  key={kpi.label}
                  className="bg-card-nested rounded-sm border border-border p-3"
                >
                    ...
                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-secondary">
                      <div
                        className={cn(
                          ...
                          RISK_TEXT[zone.risk].replace("text-", "bg-")
                        )}
                        style={{ width: `${bar.val}%` }}
                      />
                    </div>
            ...
            <div className="space-y-1.5">
              ...
              {[
                ...
                {
                  label: "Area",
                  value: `${(zone.population / 3000).toFixed(1)} km²`,
                },
                {
                  label: "Barangays",
                  value: Math.round(zone.population / 4500) + " units",
                },
                ...
                >
                  ...
                  <span
                    className={cn(
                      "font-mono",
                      row.label === "Alert Level"
                        ? RISK_TEXT[zone.risk] + " font-medium"
                        : "text-foreground"
                    )}
                  >
                    {row.value}
          ...
          <div className="space-y-2 p-4">
            {zoneIncidents.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-2 py-12">
                <CheckCircle className="text-success/50 h-8 w-8" />
                <p className="font-mono text-xs text-muted-foreground">
                  No incidents logged for this zone
                </p>
              </div>
            ) : (
              zoneIncidents.map((inc) => (
                <div
                  key={inc.id}
                  className="bg-card-nested space-y-2 rounded-sm border border-border p-3"
                >
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <Badge
                      label={inc.severity}
                      className={`bg-${SEV_DOT[inc.severity]?.replace("bg-", "") || "muted"}/10 ${SEV_TEXT[inc.severity]} border-border`}
                    />
                    <Badge
                      label={inc.status}
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 p-4">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full rounded-sm border border-border bg-input px-2.5 py-1.5 font-mono text-xs text-foreground focus:ring-1 focus:ring-primary/50 focus:outline-none"
              >
                {[
                  "Fire",
                  "Flood",
              ...
              <div className="flex gap-1.5">
                {["Critical", "High", "Medium", "Low"].map((s) => (
                  <button
                    ...
                    className={cn(
                      "flex-1 rounded-sm border py-1.5 font-mono text-[10px] transition-colors",
                      form.severity === s
            ...
            <div className="space-y-1">
              ...
              <input
                value={zone.label}
                readOnly
                className="w-full cursor-not-allowed rounded-sm border border-border bg-secondary px-2.5 py-1.5 font-mono text-xs text-muted-foreground"
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-sm border border-border bg-input px-2.5 py-1.5 font-mono text-xs text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-primary/50 focus:outline-none"
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full resize-none rounded-sm border border-border bg-input px-2.5 py-1.5 font-mono text-xs text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-primary/50 focus:outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      responders: Number(e.target.value),
                    }))
                  }
                  className="w-full rounded-sm border border-border bg-input px-2.5 py-1.5 font-mono text-xs text-foreground focus:ring-1 focus:ring-primary/50 focus:outline-none"
                />
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full rounded-sm border border-border bg-input px-2.5 py-1.5 font-mono text-xs text-foreground focus:ring-1 focus:ring-primary/50 focus:outline-none"
                >
                  {["Active", "Responding", "Contained", "Resolved"].map(
                    (s) => (
            ...
            <button
              type="submit"
              className={cn(
                "w-full rounded-sm py-2 text-xs font-semibold transition-colors",
                submitted
